...
Cube::Cube(int cubeSize, int positionX, int positionY)
{
    this->positionX = positionX;
    this->positionY = positionY;
    this->cubeSize = cubeSize;

    //vertices.push_back(Point3D(-cubeSize, 0, -cubeSize));
    //vertices.push_back(Point3D(0, cubeSize, -cubeSize));
}

// dodanie wierzchołka do siatki, ustalenie pozycji na kuli
int Cube::addVertex(Point3D p)
{
    //double length = sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
    //vertices.push_back(Point3D(p.x/length, p.y/length, p.z/length));
    vertices.push_back(Point3D(p.x, p.y, p.z));
    return index++;
}

// zwraca indeks punktu w środku p1 i p2
int Cube::getMiddlePoint(int p1, int p2)
{
...
void Cube::create(int recursionLevel)
{
    ...
    addVertex(Point3D(cubeSize, cubeSize, -cubeSize));
    addVertex(Point3D(-cubeSize, cubeSize, -cubeSize));
    addVertex(Point3D(-cubeSize, -cubeSize, -cubeSize));
    addVertex(Point3D(cubeSize, -cubeSize, -cubeSize));
    ...
    // stworzenie 12 trojkatow
    // przod
    faces.push_back(TriangleIndices(1, 0, 2));
    faces.push_back(TriangleIndices(3, 2, 0));
    // tyl
    faces.push_back(TriangleIndices(6, 7, 5));
    faces.push_back(TriangleIndices(4, 5, 7));
    // lewo
    faces.push_back(TriangleIndices(1, 2, 5));
    faces.push_back(TriangleIndices(6, 5, 2));
    // prawo
    faces.push_back(TriangleIndices(0, 4, 3));
    faces.push_back(TriangleIndices(7, 3, 4));
    // dol
    faces.push_back(TriangleIndices(0, 1, 4));
    faces.push_back(TriangleIndices(5, 4, 1));
    // gora
    faces.push_back(TriangleIndices(2, 3, 6));
    faces.push_back(TriangleIndices(7, 6, 3));
...
void Cube::draw(QImage *img, Texture *texture, int f_ogniskowa, Point3D POV, Light light, Transform transform, bool drawEdges, bool drawTextures)
{
    ...
    for(int i = 0; i < faces.size(); i++)
    {
        ...
        Point3D w1, w2, wp, w1_w2_normal, w_light;

        // wektor od wierzcholka do obserwatora
        wp.x = POV.x - p1.x; wp.y = POV.y - p1.y; wp.z = POV.z - p1.z;
        ...
        // wektor od punktu do swiatła
        w_light.x = light.x - p1.x; w_light.y = light.y - p1.y; w_light.z = light.z - p1.z;
        ...
        // "wartosc" oswietlenia
        double lightValue = (w1_w2_normal.x * w_light.x + w1_w2_normal.y * w_light.y + w1_w2_normal.z * w_light.z) * light.intensity;
        if(lightValue < 0)
            lightValue = 0;
        ...
        if((w1_w2_normal.x * wp.x + w1_w2_normal.y * wp.y + w1_w2_normal.z * wp.z) > 0)
        {
            ...
            if(drawTextures)
            {
                QVector<QPoint> points, texturePoints;
